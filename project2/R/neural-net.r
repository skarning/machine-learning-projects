...
neuralNetModel <- neuralnet("buying.high + buying.low + buying.med + buying.vhigh ~ .", data=encodedTrain, hidden=3, linear.output = FALSE)
...
predictionResult <- data.frame(c("Training data", "Test data", "Cross validation"), c(trainDataAccuracy, testDataAccuracy, crossValidationAccuracy))
